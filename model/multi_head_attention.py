...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        for i in range(num_heads):
            self.attn_heads.append(self.SingleHeadAttention(embedding_dim, head_size))
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        for head in self.attn_heads:
            prelim_outs.append(head(embedded))
        out_concat = torch.cat(prelim_outs, dim=2)
        out_proj = self.output_proj(out_concat)
        return torch.round(out_proj, decimals=4)
